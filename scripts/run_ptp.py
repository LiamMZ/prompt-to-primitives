...
def build_molmo() -> Any:
    """Load Molmo2-4B once; shared between detect_objects and PrimitiveExecutor."""
    from ptp.perception.molmo_point_detector import MolmoPointDetector
    logger.info("Loading Molmo2-4B …")
...
def save_run_outputs(
    ...
    if plan is not None:
        for i, prim in enumerate(plan.primitives):
            ...
            ref_id = prim.references.get("object_id", f"step{i}")
            c = _obj_colour(ref_id)
            r = max(8, iw // 80)

            if pos2d is not None and len(pos2d) >= 2:
            ...
            # Also draw 3D-projected position if available (back-project not needed,
            # just label the step near the bbox centroid of its reference object)
            elif prim.metadata.get("molmo_position_3d") is not None:
    ...
    logger.info(
        ...
        "(detections.png, interaction_points.png, plan.json, "
        "llm_response.json, timings.json, run_summary.json)",
        run_dir,
    )
...
def main() -> None:
    ...
    input("\nPerception complete — press Enter to continue to decomposition…\n")
